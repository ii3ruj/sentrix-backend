"""
SentriX Backend API & Real-Time AI Decision Engine (v7.0 - Ultimate Fixes)
---------------------------------------------------------------------------
Intelligent Simulator, Dynamic CRSI, Trends, Rich Playbooks, and SOAR.
"""
...
SIM_INTERVAL = int(os.environ.get("SIM_INTERVAL_SECONDS", "30"))
...
app = FastAPI(title="SentriX AI Engine", version="7.0.0")
app.add_middleware(
...
# ===========================================================================
# 3. STATIC DICTIONARIES (FALLBACKS TO FIX ISSUES 4 & 5)
# ===========================================================================
CRSI_DOMAINS = {
...
PLAYBOOKS = {
    "ransomware": {"name": "RANSOMWARE_RESPONSE_PLAYBOOK", "actions": [("Isolate affected host", "Disconnect the affected host from the network.", "HIGH"), ("Block malicious IP", "Add identified IPs to the firewall.", "HIGH"), ("Identify affected files", "Check for encrypted directories.", "MEDIUM")]},
    ...
    "phishing": {"name": "PHISHING_RESPONSE_PLAYBOOK", "actions": [("Quarantine the email", "Remove message from mailboxes.", "HIGH"), ("Block sender domain", "Add sender domain to blocklist.", "HIGH"), ("Notify affected users", "Inform users to ignore the email.", "LOW")]},
    "malware": {"name": "MALWARE_RESPONSE_PLAYBOOK", "actions": [("Isolate the endpoint", "Isolate infected endpoint.", "HIGH"), ("Run full antivirus scan", "Execute a full scan.", "HIGH"), ("Remove malicious binaries", "Delete persistence entries.", "MEDIUM")]},
    "insider_threat": {"name": "INSIDER_THREAT_RESPONSE_PLAYBOOK", "actions": [("Suspend user access", "Temporarily suspend account access.", "HIGH"), ("Review data access history", "Review accessed data over last 30 days.", "MEDIUM"), ("Notify HR and legal", "Escalate to stakeholders.", "MEDIUM")]},
...
def notify_twilio(ref: str, severity: str, incident_type: str, risk_score: int) -> dict:
    if severity != "Critical": return {"sent": False}
    if not (TWILIO_SID and TWILIO_TOKEN and TWILIO_PHONE and TEAM_NUMBERS): return {"sent": False}
    try:
        from twilio.rest import Client
        client = Client(TWILIO_SID, TWILIO_TOKEN)
        body = f"SentriX ALERT: Critical {incident_type} incident {ref}. Risk {risk_score}/100. Immediate response required."
        for num in TEAM_NUMBERS: client.messages.create(body=body, from_=TWILIO_PHONE, to=num)
        return {"sent": True}
...
def compute_crsi(packages: list) -> dict:
    ...
    return {"score": overall, "maturity_level": maturity, "breakdown": sorted(breakdown, key=lambda b: b["score"]), "incident_count": len(window)}
...
def process_incident(payload: IncidentIn) -> dict:
    ...
    if payload.flow_features:
        try:
            ...
            ai_data.update({
                "anomaly_score": pred.get("anomaly_score"), "is_anomaly": pred.get("is_anomaly", False),
                "model_name": pred.get("model_name", "Isolation Forest"), "dynamic_threshold": pred.get("prediction_metadata", {}).get("threshold", 0.1167)
            })
        except: pass
    ...
    fallback_domains = MITRE_MAP.get(itype, MITRE_MAP["_default"])["domains"]
    ...
    threat = {
        ...
        "cia_impact": {"confidentiality": str(threat_result.get("confidentiality_impact", "Medium")).capitalize(), "integrity": str(threat_result.get("integrity_impact", "Medium")).capitalize(), "availability": str(threat_result.get("availability_impact", "Medium")).capitalize()},
        "failed_domains": fallback_domains  # Fix Issue 5: Dynamic domain failing
    }

    # Fix Issue 4: Dynamic Playbooks
    ...
    findings = [narrative.get("analysis_summary", f"Incident analyzed with severity {risk['severity']}."), f"Risk scored {risk['risk_score']}/100."]
    ...
    package["report"] = {"report_id": f"RPT-{ref.replace('INC-', '')}", "generated_at": created_at, "report_version": "7.0"}
    ...
    # Save to Supabase
    sb_insert("incidents", {"id": incident_uuid, "title": incident_row["title"], "source": incident_row["source"], "incident_type": incident_row["incident_type"], "created_at": created_at})
    sb_insert("incident_reports", {"id": str(uuid.uuid4()), "incident_id": incident_uuid, "report_json": package, "pdf_path": package["archive"]["pdf_path"], "report_version": "7.0"})
...
def render_pdf(pkg: dict) -> bytes:
    ...
    story = [Paragraph("SentriX — Cybersecurity Incident Report", styles["Title"]), Spacer(1, 14), Paragraph(f"Report ID: {pkg['report']['report_id']}", styles["Normal"])]
    doc.build(story)
    return buf.getvalue()
...
async def dashboard_stats():
    ...
    # Fix Issue 2: Trends calculation
    ...
    def pct(a, b, is_bad=True):
        if not b: return {"change": "100%" if a else "0%", "positive": len(a) <= len(b) if is_bad else len(a) >= len(b)}
...
async def get_incident(incident_id: str):
    ...
    return {**p["incident"], "risk_score": p["risk"]["risk_score"], "severity": p["risk"]["severity"], "priority": p["risk"]["priority"], "sla_hours": p["risk"]["sla_hours"], "scoring_mode": p["risk"]["scoring_mode"], "flow": p["risk"]["flow"], "risk_factors": p["risk"]["risk_factors"], "anomaly_score": p["ai_result"]["anomaly_score"], "model_used": p["ai_result"]["model_name"], "dynamic_threshold": p["ai_result"]["dynamic_threshold"], "mitre_tactics": ", ".join(p["threat"]["mitre_tactics"]) or "N/A", "attack_technique": ", ".join(p["threat"]["mitre_techniques"]) or "N/A", "cia_impact": p["threat"]["cia_impact"], "key_findings": p["key_findings"], "playbook": p["recommendation"]["playbook"], "recommended_actions": p["recommendation"]["actions"], "crsi": p["crsi"], "report": p["report"], "archive": p["archive"], "pdf_url": p["archive"]["pdf_path"], "hasAiResult": True}
...
async def ai_analysis(incident_id: str):
    ...
    return {"incident_id": p["incident"]["id"], "incident_title": p["incident"]["title"], "severity": p["risk"]["severity"], "risk_score": p["risk"]["risk_score"], "risk_detected": p["risk"]["flow"] == "full_path", "analysis_id": f"AI-ANL-{p['incident']['id']}", "model_used": p["ai_result"]["model_name"], "analysis_time": p["report"]["generated_at"], "data_sources": f"{p['incident']['source']}, Threat Intel", "mitre_tactics": ", ".join(p["threat"]["mitre_tactics"]) or "N/A", "attack_technique": ", ".join(p["threat"]["mitre_techniques"]) or "N/A", "cia_impact": p["threat"]["cia_impact"], "key_findings": p["key_findings"], "anomaly_score": p["ai_result"]["anomaly_score"], "threat_type": p["incident"]["incident_type"]}
...
async def crsi_assessment():
    crsi = compute_crsi(PACKAGES)
    return {"score": crsi["score"], "maturity_level": crsi["maturity_level"], "breakdown": crsi["breakdown"], "dailyScores": [{"date": (datetime.now(timezone.utc).date() - timedelta(days=i)).strftime("%b %d, %Y"), "score": random.randint(65, 85), "status": "Good"} for i in range(5)], "incident_count": crsi["incident_count"]}

@app.get("/api/crsi-recommendations")
async def crsi_recommendations():
    crsi = compute_crsi(PACKAGES)
    weak = [d["name"] for d in crsi["breakdown"] if d["is_weak"]]
    # Dynamic CRSI Recommendations based on Weak domains
    actions = []
    idx = 1
    for d in crsi["breakdown"]:
        if d["is_weak"]:
            ...
            idx += 1
    if not actions: actions.append({"id": 1, "title": "Maintain Security Posture", "description": "No critical weaknesses detected.", "priority": "Low", "status": "Pending"})
...
async def list_archive():
    rows = [{**p["archive"], "content": {"incidentTitle": p["incident"]["title"], "severity": p["risk"]["severity"], "riskScore": f"{p['risk']['risk_score']} / 100", "source": p["incident"]["source"], "asset": p["incident"]["asset_type"], "threatType": p["incident"]["incident_type"], "keyFindings": p["key_findings"], "playbook": p["recommendation"]["playbook"], "recommendedActions": [a["title"] for a in p["recommendation"]["actions"]]}} for p in PACKAGES]
    if PACKAGES:
        crsi = compute_crsi(PACKAGES)
...
@app.get("/health")
async def health(): return {"status": "ok", "version": "7.0.0", "incidents": len(PACKAGES)}

@app.get("/api/debug/config")
async def debug_config(): return {"supabase_connected": supabase is not None, "packages_in_memory": len(PACKAGES), "status": "Ready"}

# ===========================================================================
# 8. DYNAMIC SIMULATOR (FIXES ISSUES 1 & 3)
# ===========================================================================
...
def build_sim_incident() -> IncidentIn:
    # 1. التوزيع العادل لتقليل عدد الـ Critical
    types = ["benign", "phishing", "brute_force", "malware", "ddos", "ransomware", "insider_threat"]
    weights = [40, 25, 15, 10, 5, 3, 2] # التركيز على العادي والمتوسط
    itype = random.choices(types, weights=weights)[0]
    
    # تحديد الخطورة بناءً على نوع الهجمة
    if itype in ["benign", "phishing"]:
        crits = ["low", "medium"]
        hot = False
    elif itype in ["brute_force", "insider_threat"]:
        crits = ["medium", "high"]
        hot = False
    else:
        crits = ["high", "critical"]
    ...
    # 3. تنويع المصادر (إضافة SOAR و WAF)
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    payload = IncidentIn(title=f"Incident report — {file.filename}", incident_type=itype, source="PDF Report", input_method="pdf", source_ip=src_ip, asset_type="Server", asset_criticality="high", exposure="internet_facing" if itype in ("ddos", "ransomware") else "internal", vulnerability_level="high", business_impact="high", flow_features=extracted if complete else None)
